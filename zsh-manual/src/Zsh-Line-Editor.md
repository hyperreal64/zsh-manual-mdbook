...
- [18 Zsh Line Editor](#18-zsh-line-editor)
  ...
  - [18.4 Zle Widgets](#184-zle-widgets)
...
<span id="index-line-editor"></span> <span
id="index-editor_002c-line"></span> <span id="index-ZLE-1"></span>
...
<span id="index-BAUD_002c-use-of"></span> <span
id="index-COLUMNS_002c-use-of"></span> <span
id="index-LINES_002c-use-of"></span>
...
-M `keymap`  
The `keymap` specifies a keymap name that is selected for any operations
...
Make the `new-keymap` name an alias for `old-keymap`, so that both names
...
name, it is deleted. If an `old-keymap` name is given, the new keymap is
...
from the emacs keymap (which remains unchanged) is:
...
to command mode becomes impossible.
...
editor. When -R is also used, interpret the `in-string`s as ranges.

Note that both `in-string` and `out-string` are subject to the same form
of interpretation, as described below.

`in-string command` ...  
Bind each `in-string` to each `command`. When -R is used, interpret the
`in-string`s as ranges.
...
<span id="index-vared"></span> <span
id="index-parameters_002c-editing"></span> <span
id="index-editing-parameters"></span>
...
The -M option gives a keymap to link to the main keymap during editing,
...
Vared calls the usual ‘zle-line-init’ and ‘zle-line-finish’ hooks before
and after it takes control. Using the -i and -f options, it is possible
to replace these with other custom widgets.
...
id="index-defining-widgets"></span>
...
When combined with the -a option, all widget names are listed, including
the builtin ones. In this case the -L option is ignored.

If at least one `string` is given, and -a is present or -L is not used,
nothing will be printed. The return status will be zero if all `string`s
are names of existing widgets and non-zero if at least one `string` is
...
user-defined widgets matching any `string` are listed in the form of zle
...
Make the `new-widget` name an alias for `old-widget`, so that both names
...
For further information, see [Zle Widgets](#Zle-Widgets).
...
Create a user-defined completion widget named `widget`. The completion
widget will behave like the built-in completion-widget whose name is
given as `completion-widget`. To generate the completions, the shell
function `function` will be called. For further information, see
[Completion Widgets](Completion-Widgets.html#Completion-Widgets).

-R \[ -c \] \[ `display-string` \] \[ `string` ... \]  
Redisplay the command line. If a `display-string` is given and not
empty, this is shown in the status line (immediately below the line
being edited).

If the optional `string`s are given they are listed below the prompt in
the same way as completion lists are printed. If no `string`s are given
but the -c option is used such a list is cleared.

Note that immediately after returning from running widgets, the command
...
-M `string`  
As with the -R option, the `string` will be displayed below the command
...
that the `string` will still be displayed after the widget returns
(until it is overwritten by subsequent commands).

-U `string`  
This pushes the characters in the `string` onto the input stack of ZLE.
...
in each `string` will be processed in the order in which they appear in
the string.
...
file descriptor `fd`. Installing a handler for an `fd` which is already
...
`fd`, zle calls `handler` with the fd which is ready for reading as the
...
system call always use ‘err’.

If the option -w is also given, the `handler` is instead a line editor
widget, typically a shell function made into a widget using ‘zle -N’. In
that case `handler` can use all the facilities of zle to update the
current editing line. Note, however, that as handling `fd` takes place
...
TCP port is created using the ztcp command; see [The zsh/net/tcp
Module](Zsh-Modules.html#The-zsh_002fnet_002ftcp-Module). Then a handler
...
module may not even be loaded; if it is not, the command can be skipped.

It is possible to call ‘zle -I’ several times before control is returned
...
‘zle -Tr `transformation`’ removes the given `transformation` if it was
present (it is not an error if none was).
...
With the options -n and -N, the current numeric argument will be saved
and then restored after the call to `widget`; ‘-n `num`’ sets the
numeric argument temporarily to `num`, while ‘-N’ sets it to the
default, i.e. as if there were none.

With the option -K, `keymap` will be used as the current keymap during
...
<span id="Zle-Widgets"></span> <span id="Zle-Widgets-1"></span>
...
Widgets](#Standard-Widgets). Other built-in widgets can be defined by
other modules (see [Zsh Modules](Zsh-Modules.html#Zsh-Modules)). Each
...
<span id="User_002dDefined-Widgets"></span> <span
id="User_002dDefined-Widgets-1"></span>
...
<span id="index-parameters_002c-editor"></span> <span
id="index-parameters_002c-zle"></span>
...
the appropriate end of the buffer.
...
<span id="index-ISEARCHMATCH_005fACTIVE"></span> <span
id="index-ISEARCHMATCH_005fSTART"></span> <span
id="index-ISEARCHMATCH_005fEND"></span>
...
-   Optionally, a string of the form ‘memo=`token`’. The `token`
    ...
    then use ‘region_highlight=( ${region_highlight:#\*memo=`token`} )’
    in order to remove array elements it have added.

    (This example uses the ‘${`name`:#`pattern`}’ array-grepping syntax
...
Note that zsh 5.8 and older do not support the ‘memo=`token`’ field and
...
<span id="index-SUFFIX_005fACTIVE"></span> <span
id="index-SUFFIX_005fSTART"></span> <span
id="index-SUFFIX_005fEND"></span>
...
reading when SUFFIX_ACTIVE is non-zero.
...
<span id="index-YANK_005fACTIVE"></span> <span
id="index-YANK_005fSTART"></span> <span id="index-YANK_005fEND"></span>
...
<span id="index-ZLE_005fRECURSIVE"></span>
...
<span id="Standard-Widgets"></span> <span
id="Standard-Widgets-1"></span>
...
<span id="index-vi_002dforward_002dblank_002dword"></span>
...
<span id="index-end_002dof_002dline_002dhist"></span>
...
no effect outside incremental search.
...
<span
id="index-history_002dincremental_002dpattern_002dsearch_002dforward"></span>
...
<span id="index-backward_002ddelete_002dword"></span>
...
<span id="index-copy_002dregion_002das_002dkill"></span>
...
copy-region-as-kill `string`’ then `string` will be taken as the text to
...
<span id="index-kill_002dword"></span>
...
is followed by -2, then forward-char, move backward two spaces.
...
are not usually bound to digit-argument.
...
<span id="index-expand_002dor_002dcomplete"></span>
...
<span id="index-menu_002dcomplete"></span> <span
id="index-MENU_005fCOMPLETE_002c-use-of"></span>
...
<span id="index-end_002dof_002dlist"></span>
...
`cmd`’, where `cmd` is the current command. run-help is normally aliased
to man.
...
hole’ buffer "\_. The named buffers can also be specified as "A to "Z.
...
<span id="index-set_002dmark_002dcommand"></span>
...
<span id="index-visual_002dline_002dmode"></span>
...
‘which-command `cmd`’. where `cmd` is the current command. which-command
is normally aliased to whence.

<span
id="index-vi_002ddigit_002dor_002dbeginning_002dof_002dline"></span>
...
<span id="Character-Highlighting"></span> <span
id="Character-Highlighting-1"></span>
...
<span id="index-region_002c-highlighting"></span> <span
id="index-highlighting_002c-region"></span>
...
vi mode, the region corresponds to selected text in visual mode.

<span id="index-special-characters_002c-highlighting"></span> <span
id="index-highlighting_002c-special-characters"></span>
...
<span
id="index-removable-suffix_002c-highlighting-in-completion"></span>
...
fg=`colour`  
The foreground colour should be set to `colour`, a decimal integer, the
...
may generate additional colours if the bold attribute is also present.
...
zsh/nearcolor module (see [The zsh/nearcolor
Module](Zsh-Modules.html#The-zsh_002fnearcolor-Module)).

Colour is also known as color.

bg=`colour`  
The background colour should be set to `colour`. This works similarly to
...
whitespace will not be highlighted.
...
where a combining character is not valid, the character

angle brackets. The number is the code point of the character in the
...
special array parameter region_highlight; see [Zle
Widgets](#Zle-Widgets).
